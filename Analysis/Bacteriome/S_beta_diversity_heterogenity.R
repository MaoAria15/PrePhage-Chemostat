...
ggplot(combined_dis, aes(x = group, y = dissimilarity))+
  ...
  stat_pvalue_manual(stat,label = "p", tip.length = 0, size = 4,
...
write.csv(stat,file = "stat_result/Simone/heterogenity_bray_litter123.csv")
...
combined_dis <- data.frame(group = rep(c("CVT_MO", "CON"), times = c(length(CVT_MO_var),length(CON_var))),
                           dissimilarity = c( CVT_MO_var,CON_var))



combined_dis$group <- factor(combined_dis$group ,levels = c("CVT_MO", "CON"))
...
write.csv(stat,file = "stat_result/Simone/heterogenity_bray_litter45.csv")
