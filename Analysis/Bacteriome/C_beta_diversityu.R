...
ps<-  subset_samples(PSB.CSS,Substrate %in% "HMO")
Method="bray"
...
ps<-  subset_samples(PSB.CSS,Substrate %in% "Lactose")
Method="bray"
...
ps@sam_data$Substrate <- factor(ps@sam_data$Substrate, levels = c("no","HMO","Lactose"))
...
###################################################################################
write_xlsx(sheet, "Bacteriome/stat_result/Chemostat/beta_diversity.xlsx")
