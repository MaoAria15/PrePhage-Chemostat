...
Groups <- Groups[Groups!="FVT"]
path_table <- "Bacteriome/stat_result/Malene/Deseq/"
dir.create(path_table)

for (group in Groups){
  ps.sub <- prune_samples(sample_data(ps)$Group %in% c(group, "FVT"), ps)
  ps.sub
  ...
  write.table(data.frame(sigtab), paste0(path_table,str_replace(group," ",""),"_FVT.tsv"), sep="\t", col.names = NA)
}
