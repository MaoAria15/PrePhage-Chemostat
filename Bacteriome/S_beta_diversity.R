...
ps<-  subset_samples(PSB.CSS,Litter %in% c("1","2","3"))
Method="bray"
...
p_HMO <- phyloseq::plot_ordination(ps, GP.ord, axes = 1:2,color="Group") + 
  ...
  scale_color_manual(values= simone_cols)+
...
t__Litter_123 <- data.frame(Group1=c("CON","CON","CON","FVT","FVT","CVT_MO"),
                     Group2=c("FVT","CVT_MO", "CVT","CVT_MO","CVT","CVT"),
                     R2= round(r,digits = 3),
                     F=round(f,digits = 3),
                     p=round(p,digits = 3),
...
t__Litter_123
sheet[["Litter_123"]] <- t__Litter_123
##############Litter 45############################

ps<-  subset_samples(PSB.CSS,Litter %in% c("4","5"))
Method="bray"
...
adonis_p <- adonis_sub[1,4]
ps@sam_data$Group <- factor(ps@sam_data$Group, levels = S_groups)
p_Litter_45 <- phyloseq::plot_ordination(ps, GP.ord, axes = 1:2,color="Group") + 
  stat_ellipse(geom = "polygon", level = 0.95, fill = NA, linewidth = 1,show.legend = FALSE) +
  geom_point(alpha=2, size=2,aes(color=Group)) +
  scale_color_manual(values= simone_cols[3:4])+
  ggtitle(paste( "Litter 123\n",
                 "R2=", format(adonis_R2[1],digits = 2),", P=",format(adonis_p[1], digits = 1),"\n")) +
  theme_classic()+
  theme(plot.title = element_text(hjust = 0.5))+
...
t__Litter_45 <- data.frame(Group1=c("CVT_MO"),
                            Group2=c("CON"),
                            R2= round(r,digits = 3),
                            F=round(f,digits = 3),
                            p=round(p,digits = 3),
...
t__Litter_45
sheet[["Litter_45"]] <- t__Litter_45
###########################################

write_xlsx(sheet,"Bacteriome/stat_result/Simone/beta_diversity.xlsx")
