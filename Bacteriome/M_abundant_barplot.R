...
#Find top 30 tax
top
top20 <- top$tax[1:20] #Choose number of taxa you want to include in the result
...
n3 <-  count(ps1@sam_data$Group == "SPC")[[2,2]]
...
df0$Sample <- factor(df0$Sample, levels = M_groups)
...
p_barplot <- ggplot(df0, aes(Sample, Abundance, fill = tax)) + 
  ...
  mytheme_abundance+
  labs(x= "",y="Mean Relative abundance (%)",  title="")+
