...
L_123 <- ggplot(tab_sub, aes(x= Group, y= Shannon)) +
...
L_45 <- ggplot(tab_sub, aes(x= Group, y= Shannon)) +
  stat_boxplot(geom ='errorbar', linetype=1, width=0.2) + 
  geom_boxplot(outlier.shape = NA, aes(fill=Group)) +
  geom_point(position = position_jitter(w=0.1, h=0),size=2, aes(colour=NEC)) +
  scale_color_manual(values = NEC_col) +
...
L_123_nec <- ggplot(tab_sub, aes(x= NEC, y= Shannon)) +
  stat_boxplot(geom ='errorbar', linetype=1, width=0.2) + 
...
L_45_nec <- ggplot(tab_sub, aes(x= NEC, y= Shannon)) +
  stat_boxplot(geom ='errorbar', linetype=1, width=0.2) + 
  geom_boxplot(outlier.shape = NA, aes(fill=NEC)) +
  geom_point(position = position_jitter(w=0.1, h=0),size=2) +
  scale_fill_manual(values = NEC_col) +
...
L_12345_nec <- ggplot(tab_sub, aes(x= NEC, y= Shannon)) +
  stat_boxplot(geom ='errorbar', linetype=1, width=0.2) + 
  geom_boxplot(outlier.shape = NA, aes(fill=NEC)) +
  geom_point(position = position_jitter(w=0.1, h=0),size=2) +
  scale_fill_manual(values = NEC_col) +
  ...
  labs(x="", y="Shannon diversity index", title="16S rRNA") +
  # labs(x="", y="Shannon diversity index", title="16S rRNA") +
  stat_pvalue_manual(stat_bac,label = "p", tip.length = 0, size = 3,y.position = c(4.15))+
  theme_classic() +
  mytheme_beta
L_12345_nec
write_xlsx(sheet, "Bacteriome/stat_result/Simone/alpha_diversity_shannon.xlsx")
